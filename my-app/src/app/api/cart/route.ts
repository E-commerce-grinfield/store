...
export const POST = async (req: NextRequest) => { 
  ...
  if (!cart)     return new Response(JSON.stringify({ message: "Failed to create cart"   }));
  return new Response(JSON.stringify({ message: "cart   created successfully" }),{status:201});
...
export const GET = async (req: NextRequest) => {
    ...
    try {
      ...
      const fav = await prisma.carts    .findMany({
        where: {
          userId: UserId,
        },
      });
